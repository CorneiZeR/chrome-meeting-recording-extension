...
1. Create the database: `npx wrangler d1 create recording-extension-telemetry` and replace the placeholder database ID in `wrangler.toml`.
...
3. Apply migrations remotely only after review: `npx wrangler d1 migrations apply recording-extension-telemetry --remote`.
4. Run tests, typecheck, and the Wrangler dry run, then deploy the Worker.
5. Set the extension build's `TELEMETRY_ENDPOINT` to `https://recording-extension-telemetry.kstroevsky.workers.dev/api/telemetry/batches` and test from the real unpacked extension.
