...
- **Orphan recovery** (`recoverOrphanRecordings.ts`) reclaims OPFS bytes left behind when the offscreen document dies after writing but before delivery — the counterpart to every "kept on disk for orphan recovery" escape hatch above. Which leftovers it adopts is decided by `isRecordingFilename` from [`shared/recordingNames`](../../shared/README.md): the same parser that groups a run's artifacts, so a name shape it does not recognize is a recording nobody comes back for.
