...
- **Captions cannot be read unless Meet is generating them.** There is no hidden transcript source: Meet renders caption text only while its own control is on. `ensureCaptionsEnabled` therefore clicks that control (found the same language-independent way as everything else) and waits for the region to attach, so a recording is not silently transcript-less. It resolves `false` rather than throwing — a recording must never depend on it.
